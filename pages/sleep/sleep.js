...
Page({
  ...
  get:function(data){
    app.request('POST', '/sleep_day/', data, res => {
      console.log(res);
      if (res.data.code == 200) {
        console.log(res.data.data);
        let data=[320,220,220,250,720]
        this.init_one(data);
  ...
  init_one1: function (list,data1,data2) {           //饼图
    this.oneComponent1.init((canvas, width, height,dpr) => {
        ...
        setOption1(chart,list,data1,data2)
        this.chart = chart;
        return chart;
    });
  },
  ...
  onShow: function () {
    ...
    let end=new Date().setHours(0, 0, 0, 0)+86400000;
    this.get({
      start,end
    });
    this.setData({
